...
class HomeostaticEnv(gym.Env):
    ...
    - pain_loc: a specific location which can hurt stats (not implemented)
    """
    ...
    def __init__(self,
        ...
        self.n_pain_stats = n_pain_stats
        ...
        self.border = -0.02
        self.n_statedims = (self.n_stats + self.n_pain_stats) * self.view_size ** 2 + (self.n_stats + self.n_pain_stats) # Current stats + all stats in view
        ...
        self.pain_percentile = pain_percentile
        self.pain_loc = pain_loc
    ...
    def reset_stats(self):
        ...
        self.mental_stats = []
        for i in range(self.n_pain_stats):
            self.mental_stats.append(self.AgentStats.initial_mental)

    def reset_grid(self):
        """Reset both thresholds and distribution of resources."""
        self.thresholds = self.pain_thresholds = []
        if not self.stationary:
        ...
        # add painful stimuli
        if self.n_pain_stats > 0:
            self.grid_pain = get_n_resources(grid_size=self.size, bounds=self.bounds, resources=self.n_pain_stats,
                                        radius=self.radius, offset=self.offset, var=self.variance)
            self.grid = np.vstack((self.grid, self.grid_pain*3))
        ...
        for pain_stat in range(self.n_pain_stats):
            self.pain_thresholds.append(np.percentile(self.grid[self.n_stats + pain_stat, :, :], self.pain_percentile))
    ...
    def step(self, action, ep_length):
        ...
        # Get reward
        reward, module_rewards, mental_reward = self.step_stats()
        ...
        return self.get_state(), reward, self.done, module_rewards, mental_reward
    ...
    def step_stats(self):
        ...
        self.old_mental_stats = copy.deepcopy(self.mental_stats)
        ...
        # Pain
        for i in range(self.n_pain_stats):
            self.mental_stats[i] += self.AgentStats.mental_increase
            if self.mental_stats[i] > self.AgentStats.mental_health_range[1]:
                self.mental_stats[i] = self.AgentStats.mental_health_range[1]
            # Mental health drops if pain is above threshold
            if self.grid[i + self.n_stats, self.loc[0], self.loc[1]] > self.thresholds[i + self.n_stats]:
                self.mental_stats[i] -= self.grid[i + self.n_stats, self.loc[0], self.loc[1]]
            if self.mental_stats[i] < self.AgentStats.mental_health_range[0]:
                for stat in range(self.n_stats):
                    self.stats[stat] -= self.AgentStats.stat_decrease_per_step

        # Get reward
        if self.AgentStats.reward_type == 'HRRL':
            tem_old = self.old_stats + self.old_mental_stats
            tem = self.stats + self.mental_stats
            reward = self.AgentStats.HRRL_reward(tem_old, tem)
            if self.n_pain_stats > 0:
                mental_reward = 0
                #reward = reward + mental_reward
            else:
                mental_reward = 0
        ...
        if self.AgentStats.module_reward_type == 'HRRL':
            tem_old = self.old_stats + self.old_mental_stats
            tem = self.stats + self.mental_stats
            module_rewards = self.AgentStats.separate_HRRL_rewards(tem_old, tem)

        return reward, module_rewards, mental_reward
    ...
    def get_state(self):
        ...
        """
        return torch.cat((torch.tensor(self.stats).float(), torch.tensor(self.mental_stats).float(), torch.tensor(self.view.flatten()).float())).float()
    ...
    def render(self, mode='human'):
        ...
        (Mainly for visualization)
        """
        for i in range(self.n_stats + self.n_pain_stats):
            plt.subplot(100 + 10 * (self.n_stats + self.n_pain_stats) + 1 + i)
            if i < self.n_stats:
                plt.title(f'stat {i + 1}')
            else:
                plt.title('pain')
            grid = self.grid_with_agent()[i]
            plt.imshow(grid)
            plt.xticks([])
            plt.yticks([])
        plt.figure()
